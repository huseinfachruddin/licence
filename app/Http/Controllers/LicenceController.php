...
class LicenceController extends Controller
{
    ...
    public function getLicence(Request $request){
        ...
        if ($user->hasRole('admin')) {
            $data = Licence::with('product','user','domain')->orderBy('id', 'DESC')->paginate(10);
        }else{
            $data = Licence::with('product','user','domain')->where('user_id',$user->id)->orderBy('id', 'DESC')->paginate(10);
        }
